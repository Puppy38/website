...
const birdColors = {
  yellow: ['assets/flappybird.png', 'assets/flappybird.png'], // can add multiple frames if desired
...
function flap() {
  if(gameRunning){
    birdVelocity = lift;
    flapSound.play();
    // Swap bird image for flap animation
...
if (birdY + bird.offsetHeight > 600 || birdY < 0) {
  ...
  // Bird bobbing effect
  bobAngle += 0.1;
  const bobOffset = Math.sin(bobAngle) * 2;
  bird.style.top = (birdY + bobOffset) + 'px';

  // Background parallax
  backgroundX -= backgroundSpeed;
  if(backgroundX <= -400) backgroundX = 0; // loop background
